...
The API starts with a seeded `demo` account for local testing. The dashboard Order Ticket account field is editable, but `demo` is the only default account; any other account ID returns `account_not_found` unless additional accounts are added in code.
